  ...
  const tagStats = useMemo(() => {
    ...
    filteredEntries.forEach(entry => {
      if (!entry.tags || entry.tags.length === 0) {
        ...
        const existing = stats.get(untaggedKey) || { tag: untaggedKey, totalDuration: 0, entryCount: 0 };
      ...
      entry.tags.forEach(tag => {
        const existing = stats.get(tag) || { tag, totalDuration: 0, entryCount: 0 };
        stats.set(tag, {
          ...existing,
          totalDuration: existing.totalDuration + entry.duration,
          entryCount: existing.entryCount + 1
      ...
      <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
        ...
        {tagStats.length === 0 ? (
          <div className="p-8 text-center">
            <p className="text-gray-400 text-lg">No data available for the selected period.</p>
            <p className="text-gray-500 mt-2">Try selecting a different time period or add some tagged entries.</p>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-700 p-4 rounded-lg">
                ...
                <p className="text-2xl font-mono">{formatDuration(averageTimePerEntry)}</p>
              </div>
              <div className="bg-gray-700 p-4 rounded-lg">
            ...
            <div>
              <h3 className="text-xl font-semibold mb-4">Time Distribution by Tag</h3>
                ...
                {tagStats.map((stat) => (
                  <div key={stat.tag} className="bg-gray-700 p-4 rounded-lg">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <span className="font-mono text-lg">{formatDuration(stat.totalDuration)}</span>
                    </div>
                    <div className="w-full bg-gray-600 rounded-full h-3">
                      <div 
                        className="bg-blue-600 h-3 rounded-full" 
                        style={{ width: `${(stat.totalDuration / totalTime) * 100}%` }}
                      ></div>
                    </div>
                    <div className="flex justify-between text-xs text-gray-400 mt-1">
                      <span>{Math.round((stat.totalDuration / totalTime) * 100)}% of total time</span>
                      <span>Avg: {formatDuration(Math.floor(stat.totalDuration / stat.entryCount))} per entry</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-gray-700 p-4 rounded-lg">
              ...
              <p className="text-gray-300">
                During this period, you spent most of your time on <span className="font-semibold text-white">{tagStats[0]?.tag}</span> ({formatDuration(tagStats[0]?.totalDuration)}), 
                which accounts for {Math.round((tagStats[0]?.totalDuration / totalTime) * 100)}% of your tracked time.
              </p>
              {tagStats.length > 1 && (
                <p className="text-gray-300 mt-2">
                  Your second most time-consuming activity was <span className="font-semibold text-white">{tagStats[1]?.tag}</span> ({formatDuration(tagStats[1]?.totalDuration)}), 
                  accounting for {Math.round((tagStats[1]?.totalDuration / totalTime) * 100)}% of your tracked time.
                </p>
              )}
            </div>
          </div>
