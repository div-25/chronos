...
export function downloadCSV(
  csvContent: string,
  filename: string = "chronos-export.csv"
): void {
...
function parseSegmentData(rowData: Record<string, string>): TimeSegment | null {
  ...
  if (
    ...
    (endTimeStr && !isValidDate(endTime as Date)) ||
    isNaN(duration)
  ) {
    return null;
  }
...
function convertTimeEntryToProject(
...
function parseProjectData(rowData: Record<string, string>): Partial<Project> {
  return {
    ...
    title: rowData["Title"],
    parentId:
      rowData["Parent ID"] === '"' ? null : rowData["Parent ID"] || null,
